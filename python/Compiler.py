...
class Compiler(lolcodeVisitor):
    """Compiles LOLCODE into our custom AST"""
...
def compile_file(file_path: str) -> Program:
    ...
    compiler = Compiler()
    ast = compiler.visit(tree)
    return ast

def main():
    if len(sys.argv) < 2:
        print("Usage: python Compiler.py <lolcode-file>")
        return

    ast = compile_file(sys.argv[1])
    print(f"Compiled {sys.argv[1]} into AST:")
    print(ast)
